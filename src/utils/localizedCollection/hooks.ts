...
export const useIsPublished = (refetchIntervall: number = 0) => {
  ...
  const document = useLocalizedDoc<Blog>('blog', id, false, refetchIntervall);
...
export const useHasPendingChanges = (refetchIntervall: number = 0) => {
  ...
  const document: Blog = useLocalizedDoc('blog', id, false, refetchIntervall) as Blog;
  const documentDraft: Blog = useLocalizedDoc('blog', id, true, refetchIntervall) as Blog;
