...
async function saveUserToDB(payload: BasePayload, nextAuthUser: {
  cevi_db_uuid: number,
  ...
  const userExists = await payload.count(
    {
      collection: 'users',
  ...
  if (!userExists) // save the new user to the database
    ...
    strategies: [
      {
        ...
        authenticate: async ({ headers, payload }) => {
          ...
          if (!nextAuthUser || !nextAuthUser.name ||
            !nextAuthUser.email || !nextAuthUser.cevi_db_uuid) {
            return { user: null }
          }
          ...
          */
          ...
          if (!nextAuthUser.groups?.some(
            (group: { id: number, name: string }) => group.id &&
              GROUPS_WITH_API_ACCESS.includes(group.id))) {
