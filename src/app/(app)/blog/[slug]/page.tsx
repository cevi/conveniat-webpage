...
function DateTime(props: { releaseDate: string }) {
  return <time className="text-sm text-muted-foreground flex items-center gap-1">
    <CalendarIcon className="w-4 h-4" /> {stringDateToLocale(props.releaseDate)}
  </time>
}

export async function BlogPost({ slug }: BlogPostProps) {
    ...
    <article className="mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="flex columns-2 gap-8 mt-8 mb-16">
        ...
        <div>
          ...
          <span className="text-sm text-muted-foreground mb-6 block">By {article.author.fullName}</span>

          <h1 className="text-4xl font-serif font-bold leading-tight max-w-lg">
            {article.blogH1}
          </h1>

          <div className="flex items-center gap-4 mt-4 max-w-2xl">
            {article.blogCaption}
          </div>
      ...
      <div className="mx-auto py-35 s:py-100 rounded-3xl p-12 max-w-4xl" style={{ backgroundColor: '#fffaf5' }}>
