...
const BlogDisplayClient: React.FC<{
  blog: Blog;
  locale: Locale;
}> = ({ blog, locale }): React.JSX.Element => {
  const contentExcerpt = extractTextContent(blog.content.mainContent as ContentBlock[]);
  const contentExcerptTrimmed =
    contentExcerpt.length > 150 ? contentExcerpt.slice(0, 150) + '...' : contentExcerpt;

  const url = `/${blog.seo.urlSlug}`;
