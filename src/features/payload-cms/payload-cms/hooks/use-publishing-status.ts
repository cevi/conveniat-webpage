...
export const usePublishingStatus = (): {
  ...
  }) => Promise<QueryObserverResult<PublishingData | undefined, Error>>;
} => {
  ...
  const queryFunction = async (): Promise<PublishingData | undefined> => {
  ...
  if ((modified || isSyncing) && publishingStatus) {
    publishingStatus = {
      ...publishingStatus,
      [code]: {
