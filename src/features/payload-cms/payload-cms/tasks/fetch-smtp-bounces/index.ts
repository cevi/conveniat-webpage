...
const POP3_SOCKET_TIMEOUT_MS = 30_000; // 30 seconds socket timeout
...
export const fetchSmtpBouncesTask: TaskConfig<'fetchSmtpBounces'> = {
  ...
  schedule: [
    {
      ...
      hooks: {
        beforeSchedule: async ({
          ...
          const lockTtlMs = 14 * 60 * 1000; // 14 minutes lock duration for the 15-minute slot
          ...
          await recoverStaleJobs(req, 15);

          // 2. Prevent parallel execution: check if there is an active or runnable fetchSmtpBounces job
  ...
  handler: async ({ req }: { req: PayloadRequest }): Promise<{ output: { status: string } }> => {
    ...
    const timeoutPromise = new Promise<{ output: { status: string } }>((_, reject) => {
      timeoutTimer = setTimeout(() => {
        reject(
          new Error(`fetchSmtpBounces execution timed out after ${POP3_TASK_TIMEOUT_MS / 1000}s`),
        );
      }, POP3_TASK_TIMEOUT_MS);
    });

    const executeTask = async (): Promise<{ output: { status: string } }> => {
      const { payload } = req;
      ...
      try {
        ...
        for (const { id: messageId, uid } of messagesToProcess) {
